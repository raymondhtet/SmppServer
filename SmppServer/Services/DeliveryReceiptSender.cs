...
public class DeliveryReceiptSender(ILogger<DeliveryReceiptSender> logger) : IDeliveryReceiptSender
{
    ...
    {
        logger.LogInformation("Sending delivery receipt for message {MessageId}", messageId);
        ...
        logger.LogInformation(" delivery receipt sent for message {MessageId}: {Status}", 
            messageId, status.ErrorStatus);
    }
