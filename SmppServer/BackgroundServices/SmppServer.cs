...
public class SmppServer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            {
                _serverCertificate = await _certificateManager.LoadServerCertificateAsync();
                _logger.LogInformation("SSL certificate loaded - Subject: {Subject}, Expires: {Expiry}", 
                    _serverCertificate.Subject, _serverCertificate.NotAfter);
            }
        ...
        {
            _logger.LogInformation("🔄 SMPP Server execution cancelled");
        }
        catch (Exception ex)
        {
    ...
    private async Task HandleClientAsync(TcpClient tcpClient, bool useSsl, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (useSsl && _serverCertificate != null)
            {
                ...
                _logger.LogInformation("SSL session {SessionId} established from {Endpoint}", 
                    session.Id, endpoint);
            }
            else
            {
                session = new SmppSession(tcpClient, _logger);
                ...
                session is SslSmppSession sslSession && sslSession.IsSslAuthenticated))
        {
            try
            {
                    ...
                    !_sessions.ContainsKey(session.SystemId))
                {
                    ...
                    _logger.LogInformation("Session {SessionId} registered for {SystemId}", 
    ...
    private async Task StopListenersAsync()
    {
        try
        {
            ...
            _logger.LogInformation("SMPP listeners stopped");
    ...
    private async Task AcceptConnectionsAsync(TcpListener listener, bool useSsl, CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogInformation("🔄 {ListenerType} connection acceptance cancelled", listenerType);
                break;
            }
            catch (Exception ex)
            {
    ...
    private async Task RunCleanupJob(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogInformation("Active sessions: {ActiveSessions}, Total connections: {TotalConnections}",
                    _sessions.Count, ActiveSessionsCount);
            }
            catch (OperationCanceledException)
            {
                break;
